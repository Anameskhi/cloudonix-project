...
export class CreateEditProductComponent {
  ...
  acceptedEdit = false;
  items = ['furniture', 'equipment', 'stationary', 'part'];
  value2 = true;
  ...
  addKeyValuePair(): void {
    ...
    this.additionalFields.push(keyValuePair);
  }

  get additionalFields(): FormArray {
    return this.userGroupForm
      .get('profile')
      ?.get('additionalFields') as FormArray;
  }
    ...
    if (this.isEditMode() && !this.acceptedEdit) {
      this.productService.getProduct(this.editId()).subscribe((res) => {
        ...
        if (profile?.additionalFields) {
          profile.additionalFields.forEach((field: any) =>{

            const pair = this.fb.group({
              key: [{ value: field.key, disabled: true }, Validators.required], 
              value: [field.value, Validators.required],
            });
            this.additionalFields.push(pair);
          }
          );
  ...
  submitForm(): void {
    if (!this.isEditMode()) {
      this.productService
        .addProduct(this.userGroupForm.value)
        .subscribe((res) => {
          this.router.navigate(['/products-list']);
        });
    } else {
      this.productService
        .editProduct(this.editId(), this.userGroupForm.value)
        .subscribe((res) => {
          this.router.navigate(['/products-list']);
        });
    }
